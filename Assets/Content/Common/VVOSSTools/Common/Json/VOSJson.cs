...
using System.Text;
using VVOSS.Tools.FileTools;
/*
 * http://www.opensource.org/licenses/lgpl-2.1.php
 * VOSJsonObject class v.1.4.1
 * for use with Unity
...
namespace VVOSS.Json
{
	...
	public interface IVOSJsonSirializable
	{
		void SerializeToJson(IVOSJsonObject jobj);
		void DeserializeFromJson(IVOSJsonObject jobj);
	}

	public interface IVOSJsonArray
	{
		int count { get; }		
		...
		IVOSJsonObject	AddObjectItem();
		IVOSJsonArray	AddArrayItem();

		int				GetIntItem(int index);
		float			GetFloatItem(int index);
		string			GetStringItem(int index);
		bool			GetBoolItem(int index);
		IVOSJsonObject	GetObjectItem(int index);
		IVOSJsonArray	GetArrayItem(int index);		

		void SetItem(int index, bool val);
		...
		IVOSJsonObject ToJsonObject();
	}

	public interface IVOSJsonObject
	{
		int count { get; }

		IVOSJsonObject	AddObjectField(string key);
		IVOSJsonArray	AddArrayField(string key);

		void AddField(string name, bool val);
		void AddField(string name, float val);
		void AddField(string name, int val);
		...
		IVOSJsonObject	GetObjectField(string name);
		IVOSJsonArray	GetArrayField(string name);

		void SetField(string name, string val);
		void SetField(string name, bool val);
		void SetField(string name, float val);
		void SetField(string name, int val);	

		string GetKey(int index);

		void RemoveField(string name);

		IVOSJsonArray ToJsonArray();
	}
	...
	public static Queue<VOSJsonObject> releaseQueue = new Queue<VOSJsonObject>();
#endif
		...
		protected Type _type = Type.NULL;
		public int count
		{
			get
			{
		...
		public List<VOSJsonObject> list;
	...
	public float f
	{
		...
		public long i;
		public bool b;
		public delegate void AddJSONContents(VOSJsonObject self);

		public static VOSJsonObject nullJO { get { return Create(Type.NULL); } }   //an empty, null object
		public static VOSJsonObject obj { get { return Create(Type.OBJECT); } }        //an empty object
		public static VOSJsonObject arr { get { return Create(Type.ARRAY); } }     //an empty array

		public VOSJsonObject(Type t)
		{
			type = t;
			switch (t)
			{
				case Type.ARRAY:
					list = new List<VOSJsonObject>();
					break;
				case Type.OBJECT:
					list = new List<VOSJsonObject>();
		...
		public VOSJsonObject(bool b)
		{
			type = Type.BOOL;
			this.b = b;
		}
#if USEFLOAT
		public VOSJsonObject(float f)
		{
			type = Type.NUMBER;
			n = f;
		}
#else
	public VOSJsonObject(double d)
	{
		type = Type.NUMBER;
		n = d;
	}
#endif
		public VOSJsonObject(int i)
		{
			type = Type.NUMBER;
			this.i = i;
			useInt = true;
		...
		public VOSJsonObject(Dictionary<string, string> dic)
		{
			type = Type.OBJECT;
			keys = new List<string>();
			list = new List<VOSJsonObject>();
		...
		public VOSJsonObject(Dictionary<string, VOSJsonObject> dic)
		{
			type = Type.OBJECT;
			keys = new List<string>();
			list = new List<VOSJsonObject>();
			//Not sure if it's worth removing the foreach here
			foreach (KeyValuePair<string, VOSJsonObject> kvp in dic)
			{
				keys.Add(kvp.Key);
				list.Add(kvp.Value);
			}
		}
		public VOSJsonObject(AddJSONContents content)
		{
			content.Invoke(this);
		}
		public VOSJsonObject(VOSJsonObject[] objs)
		{
			type = Type.ARRAY;
			list = new List<VOSJsonObject>(objs);
		}
		//Convenience function for creating a VOSJsonObject containing a string.  This is not part of the constructor so that malformed JSON data doesn't just turn into a string object
		public static VOSJsonObject StringObject(string val) { return CreateStringObject(val); }
		public void Absorb(VOSJsonObject obj)
		{
		...
		public static VOSJsonObject Create()
		{
#if POOLING
	VOSJsonObject result = null;
	...
	if(result != null)
		return result;
#endif
			return new VOSJsonObject();
		}
		public static VOSJsonObject Create(Type t)
		{
			VOSJsonObject obj = Create();
			obj.type = t;
			switch (t)
			{
				case Type.ARRAY:
					obj.list = new List<VOSJsonObject>();
					break;
				case Type.OBJECT:
					obj.list = new List<VOSJsonObject>();
		...
		public static VOSJsonObject Create(bool val)
		{
			VOSJsonObject obj = Create();
			obj.type = Type.BOOL;
			obj.b = val;
		...
		public static VOSJsonObject Create(float val)
		{
			VOSJsonObject obj = Create();
		...
		public static VOSJsonObject Create(int val)
		{
			VOSJsonObject obj = Create();
			obj.type = Type.NUMBER;
			obj.n = val;
		...
		public static VOSJsonObject Create(long val)
		{
			VOSJsonObject obj = Create();
			obj.type = Type.NUMBER;
			obj.n = val;
		...
		public static VOSJsonObject CreateStringObject(string val)
		{
			VOSJsonObject obj = Create();
		...
		public static VOSJsonObject CreateBakedObject(string val)
		{
			VOSJsonObject bakedObject = Create();
			bakedObject.type = Type.BAKED;
			bakedObject.str = val;
			return bakedObject;
		}
		/// <summary>
		/// Create a VOSJsonObject by parsing string data
		/// </summary>
		...
		/// <param name="storeExcessLevels">Whether to store levels beyond maxDepth in baked VOSJsonObjects</param>
		...
		/// <returns></returns>
		public static VOSJsonObject Create(string val, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false)
		{
			VOSJsonObject obj = Create();
			obj.Parse(val, maxDepth, storeExcessLevels, strict);
			return obj;
		}
		public static VOSJsonObject Create(AddJSONContents content)
		{
			VOSJsonObject obj = Create();
			content.Invoke(obj);
			return obj;
		}
		public static VOSJsonObject Create(Dictionary<string, string> dic)
		{
			VOSJsonObject obj = Create();
		...
		public VOSJsonObject() { }
		#region PARSE
		public VOSJsonObject(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false)
		{   //create a new VOSJsonObject from a string (this will also create any children, and parse the whole string)
			Parse(str, maxDepth, storeExcessLevels, strict);
		}
		protected void Parse(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false)
		{
		...
		{
			AddItem(CreateStringObject(str));
		}

		public void AddItem(AddJSONContents content)
		{
			AddItem(Create(content));
		}

		public void AddItem(VOSJsonObject obj)
		{
			if (obj)
		...
		public IVOSJsonObject AddObjectItem()
		{
			var jsonobj = obj;
			AddItem(jsonobj);
			return jsonobj;
		...
		public IVOSJsonArray AddArrayItem()
		{
			var jsonobj = arr;
			AddItem(jsonobj);
			return jsonobj;
		...
		public void AddField(string name, VOSJsonObject obj)
		{
			if (obj)
			{       //Don't do anything if the object is null
				if (type != Type.OBJECT)
		...
		public IVOSJsonObject AddObjectField(string key)
		{
			var jsonobj = obj;
			AddField(key, jsonobj);
			return jsonobj;
		}

		public IVOSJsonArray AddArrayField(string key)
		{
			var jsonobj = arr;
			AddField(key, jsonobj);
			return jsonobj;
		...
		public void SetField(string name, VOSJsonObject obj)
		{
			if (HasField(name))
			{
		...
		public void SetItem(int index, bool val)
		{
			this[index] = Create(val);
		}

		public void SetItem(int index, float val)
		{
			this[index] = Create(val);
		}
		...
		public delegate void GetFieldResponse(VOSJsonObject obj);
		...
		public IVOSJsonObject GetObjectField(string name)
		{
			return GetField(name);
		}

		public IVOSJsonArray GetArrayField(string name)
		{
			return GetField(name);
		}

		public IVOSJsonArray ToJsonArray()
		{
			return this;
		}
		...
		public bool GetBoolItem(int index)
		{
			return this[index].b;
		}

		public IVOSJsonObject GetObjectItem(int index)
		{
			return this[index];
		}

		public IVOSJsonArray GetArrayItem(int index)
		{
			return this[index];
		}

		public IVOSJsonObject ToJsonObject()
		{
			return this;
		}
	...
	public bool GetField(ref double field, string name, FieldNotFound fail = null)
	{
		...
		public VOSJsonObject GetField(string name)
		{
			if (IsObject)
		...
		/// <summary>
		/// Copy a VOSJsonObject. This could probably work better
		/// </summary>
		/// <returns></returns>
		public VOSJsonObject Copy()
		{
			return Create(Print());
		}
		/*
			...
			*/
		public void Merge(VOSJsonObject obj)
		{
			MergeRecur(this, obj);
		}
		/// <summary>
		...
		static void MergeRecur(VOSJsonObject left, VOSJsonObject right)
		{
...
#pragma warning restore 219
		...
		{   //Convert the VOSJsonObject into a string
		...
		#endregion
#if UNITY_2 || UNITY_3 || UNITY_4 || UNITY_5
		public static implicit operator WWWForm(VOSJsonObject obj)
		{
...
#endif
		public VOSJsonObject this[int index]
		{
			get
			{
		...
		public VOSJsonObject this[string index]
		{
			get
			{
	...
	Debug.WriteLine
#endif
	("Tried to turn non-Object VOSJsonObject into a dictionary");
			return null;
		}
		public static implicit operator bool(VOSJsonObject o)
		{
			return o != null;
		}
#if POOLING
...
/*
		...
		int AddItem(bool val);
		int AddItem(float val);
		int AddItem(int val);
		int AddItem(string val);
	...
	public class VOSJsonUtility
	{

		public static string ToJson(IVOSJsonSirializable obj, bool pretty = false)
		{
			var json = new VOSJsonObject();
			obj.SerializeToJson(json);
			return json.Print(pretty);
		}

		public static void FromJson(IVOSJsonSirializable obj, string json)
		{
			var jsonObj = VOSJsonObject.Create(json);
			obj.DeserializeFromJson(jsonObj);
		}



		public static void ToJsonFile(string file, IVOSJsonSirializable obj, bool pretty = false)
		{
		...
		public static void FromJsonFile(string file, IVOSJsonSirializable obj)
		{
